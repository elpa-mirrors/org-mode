...
| =TIMESTAMP=    | The first active keyword-less timestamp in the entry.[fn:16]   |
...
...
[fn:58] The link is: https://orgmode.org
#+end_example
...
- =LANGUAGE= ::
  ...
  List of languages used in the document. The first element is used
  for translating certain strings (~org-export-default-language~).
  With =#+LANGUAGE: fr es=, for example, Org translates =Table of
  contents= to the French =Table des matières=[fn:42], since French is
  the first element in the list. You signal Org that there are parts
  of the document in Spanish (with the second element =es=).
...
Read sections [[*Different languages and character sets in LaTeX
export]] and [[*Font configuration for LuaLaTeX and XeLaTeX]] to
familiarise yourself with font customisation if you intend to create
PDF files from your Beamer Org documents using modern LaTeX compilers
like =lualatex= or =xelatex=.
...
The Beamer export backend has several additional keywords for
customizing Beamer output.  These keywords work similar to the general
options settings (see [[*Export Settings]]).
...
- =SUBTITLE= ::
  ...
  #+cindex: @samp{SUBTITLE}, keyword
  The document subtitle.  For typesetting, use
...
~org-latex-compiler~; for multi-language handling, see
~org-latex-multi-lang~; for build sequences, see
...
on blank lines to tell apart paragraphs.
...
The LaTeX export backend finds the compiler version to use from
~org-latex-compiler~ variable or the =#+LATEX_COMPILER= keyword in the
Org file.  See the docstring for ~org-latex-default-packages-alist~
for loading packages with certain compilers. Also see the docstring
for ~org-latex-bibtex-compiler~ to set the bibliography
compiler[fn:48].
...
which compile LaTeX files with different packages and output
options. It provides several additional keywords to
customize LaTeX output.  Setting these keywords works similar to the
...
- =LATEX_GRAPHICS_PATH= ::
  ...
  #+cindex: @samp{LATEX_GRAPHICS_PATH}
  The directories where LaTeX will find images, when they are
  referenced without path.  See ~org-latex-graphics-path~ for details.
...
- =LATEX_MULTI_LANG= ::
  ...
  The strategy to handle multilingual features.  Possible values are
  ~nil~ (default), ~fontspec~, ~polyglossia~ or ~babel~.  See more
  details about setting up multi-language documents in
  [[#lang-charset-latex]]. You can set the value globally by
  customizing ~org-latex-multi-lang~ variable. Setting the keyword
  supersedes the variable value.
...
- =:environment= ::
  ...
  Set the default LaTeX table environment for the LaTeX export backend
  to use when exporting Org tables.  Common LaTeX table environments
  are provided by these packages: =tabularx=, =longtable=, =array=,
  =tabu=, and =bmatrix=.  For packages, such as =tabularx= and =tabu=,
...
#+vindex: org-latex-graphics-path
Finally, you can configure the /graphics path/ where your images are
located. This is convenient if you add native LaTeX code that includes
images. Set the custom variable ~org-latex-graphics-path~ to the list
of directories where your images are located, using the syntax of the
[[https://latexref.xyz/_005cgraphicspath.html][LaTeX ~\graphicspath{}~
command]]. For example:

#+BEGIN_SRC emacs-lisp
(setq org-latex-graphics-path "{./}{./images}")
#+END_SRC

will include the following LaTeX code in the LaTeX preamble

#+BEGIN_SRC latex
\graphicspath{{./}{./images}}
#+END_SRC

that will allow you to include images from the Org file's directory
and the ~images~ directory under it. You can also set this variable
with keyword =LATEX_GRAPHICS_PATH=:

#+BEGIN_SRC  org
,#+LATEX_GRAPHICS_PATH: {./}{./images}
#+END_SRC

will produce the same LaTeX code in the preamble.
...
If you plan to use, for example the =Verbatim= environment for all
example blocks in a document, you can change variable
~org-latex-default-example-environment~ to the desired environment. In
the example above, you could

#+BEGIN_SRC emacs-lisp
(setq org-latex-default-example-environment "Verbatim")
#+END_SRC

to avoid having to set the environment for every example.
...
*** Table of contents in LaTeX export
...
*** Different languages and character sets in LaTeX export
:properties:
:CUSTOM_ID: lang-charset-latex
:DESCRIPTION: Localisation and font management
:end:
#+cindex: @samp{LANGUAGE} keyword, @samp{LATEX_MULTI_LANG} keyword
#+vindex: org-latex-multi-lang
#+vindex: org-export-default-language

  LaTeX was initially developed to handle documents written primarily
  in (American) English.  The standard LaTeX document classes[fn:49] assume
  this and provide a default font configuration to deliver an intended
  appearance to the output.  Spelling out dates, section headers for
  the table of contents, etc. as well as captions for figures, tables,
  listings, etc. are defined in this context.  As long as you don't
  need localization (i.e. adapting or translating these to the
  document's intended linguistic environment), you can use most LaTeX
  document classes as-is with any document that uses the Latin
  character set.  Documents using non-Latin character sets will need
  font configuration, which depend on the document class, the LaTeX
  compiler and multi-language support package you use[fn:50]. Consult
  the
  [[https://en.wikibooks.org/wiki/LaTeX/Internationalization][internationalization
  section]] of the
  [[https://upload.wikimedia.org/wikipedia/commons/2/2d/LaTeX.pdf][LaTeX
  Wikibook]] for more insight on font and language management in
  LaTeX.

  For documents that are outside the American English linguistic
  context and for multilingual documents, LaTeX provides the packages
  =babel= and =polyglossia= to localize document features like text
  direction (left-to-right vs. right-to-left), the table of contents,
  dates, hyphenation, etc.
  ...
  handle localization and font configuration when exporting to LaTeX.

  =LATEX_MULTI_LANG= is ~nil~ by default. In this case, you need to
  handle localization manually, configuring LaTeX language features in
  LaTeX headers (see ~#+LATEX_HEADER~ keyword).  Use this setting when
  you must use =pdflatex= and follow the instructions in
  [[#pdflatex-fonts]] to set up fonts in your document.

  For convenience, if the LaTeX headers load =babel= or =polyglossia=,
  you may use =AUTO= in the package options to refer to the document's
  main language. Org will translate it according to the value of the
  =#+LANGUAGE= keyword. For example:
...
#+LATEX_HEADER: \usepackage[AUTO]{babel}
  #+END_EXAMPLE
  #+texinfo: @noindent
  tells LaTeX to use the ~babel~ package to create a document with
  Spanish headers. The =LATEX_HEADER= would be translated [fn:51] to
  #+BEGIN_EXAMPLE
...
,#+LATEX_MULTI_LANG: polyglossia
  #+END_SRC
  #+texinfo: @noindent
  sets multi-language handling to use =polyglossia= in a document,
  where the main language (used for header, figure, table names and
  ...
  @@latex:\textgerman{@@Dies ist ein Abschnitt auf Deutsch.@@latex:}@@

  ,#+begin_german
  Auch dies ist ein Absatz auf Deutsch.
  ,#+end_german
  #+end_src

  For =babel=:
  #+begin_src org
  @@latex:\foreignlanguage{french}{@@Ceci est un passage en français.@@latex:}@@

  ,#+ATTR_LATEX: :options {french}
  ,#+begin_otherlanguage
  Ce passage est aussi en français.
  ,#+end_otherlanguage
  #+end_src

  Consult the package manuals to get the language name[fn:52] for the
  LaTeX markup.
  ...
  This package assumes that the document is intended for an American
  English context (i.e. header, figure, table names and typesetting
  options will be set for American English).

  ~fontspec~ distinguishes between four font families: Roman (=main=),
  Sans-Serif (=sans=), Monotype (=mono=) and Mathematical (=math=).
...
~pdflatex~ uses LaTeX packages to import the fonts used in the
PDF output. Refer to [[https://tug.org/FontCatalogue/][the LaTeX font
catalog]] to find the LaTeX package or packages you need for your
document and include them with the ~#+LATEX_HEADER:~ keyword.

For example, if you need the
[[https://tug.org/FontCatalogue/erewhon/][Erewhon]] font, you will find
the following configuration in the LaTeX font catalog:
...
By default, LaTeX export includes the ~fontenc~ package automatically
(see ~org-latex-default-packages-alist~), so you do not need to
include it in your header.  The rest of the configuration needs to be
included manually as:

#+BEGIN_SRC org
,#+LATEX_HEADER: \usepackage[proportional,scaled=1.064]{erewhon}
,#+LATEX_HEADER: \usepackage[erewhon,vvarbb,bigdelims]{newtxmath}
,#+LATEX_HEADER: \renewcommand*\oldstylenums[1]{\textosf{#1}}
#+END_SRC

*** Font configuration for LuaLaTeX and XeLaTeX
:PROPERTIES:
:custom_id: nonpdflatex-fonts
:END:
#+cindex: latex backend
#+vindex: org-latex-multi-lang
#+vindex: org-latex-classes
#+vindex: org-latex-unicode-math-options

When you use ~lualatex~ or ~xelatex~ as your =LATEX_COMPILER=, you can
control the fonts used in the PDF using the ~fontspec~ package.  To
enable optimized ~fontspec~ configuration[fn:53], set variable
~org-latex-multi-lang~ or the =LATEX_MULTI_LANG= option to ~fontspec~.
...
code in the LaTeX preamble, i.e. at the beginning of the LaTeX code
before the actual document.

When exporting to ~lualatex~ or ~xelatex~, Org will include the
=unicode-math= package to handle maths and allow equations to be
composed with UTF-8 symbols directly. Use the
~org-latex-unicode-math-options~ custom variable to adjust how this
package handles math.

Org will pass this variable as an option string to the =unicode-math=
package.  For example, to use upright fonts instead of italics in
equations, use the following settings in ~.dir-locals.el~:
...
configuration for the ~babel~ package may also use an alternative mechanism
[[#babel-fontconfig]].


**** Configuration of the ~fontspec~ package
:PROPERTIES:
:custom_id: lualatex-fontspec
:description: Using org-latex-fontspec-config
:END:
#+cindex: fontspec configuration
...
knows four font families, namely: =main= as the default (Serif) font,
=sans= for text typeset using Sans Serif fonts, =mono= for monospaced
text (for example, listings) and =math= to typeset mathematical
expressions.

Consider the following definition for ~org-latex-fontspec-config~:
...
#+texinfo: @noindent
In this examples, you are mapping the ~math~ font used by LaTeX for
mathematical expressions to the system font ~TeX Gyre Termes Math~,
using the *mandatory* property ~:font~. When generating the font
prelude for ~lualatex~, the resulting LaTeX commands will be:
...
#+texinfo: @noindent
The complete configuration reads like this: you will use the
~FreeSerif~ system font as the default font set in serif, the
~FreeSans~ system font for sans-serif passages, ~TeX Gyre Cursor~ for
monospaced passages (for example, listings) and ~TeX Gyre Termes Math~
for equations or mathematical expressions in general.

If you need extra features for any of the fonts, you need to add them
with the property ~:features~ [fn:: You may also use ~:props~ as an
alias for ~:features~. (cf. ~org-latex-fontspec-default-features~
below)].  For example, if you want a nicer scaling for the monotype
font you can define the ~mono~ font as:
...
The ~:features~ property can be a string or a list of strings to improve
readability.  For example, to make the ~mono~ font appear in blue and
scaled, you can specify it as:
...
as a comma-separated list in a single string. The following code also
would also define the ~mono~ font as blue and scaled [fn::Note that we
are using ~:props~ as an alias for ~features~.]:

#+BEGIN_SRC emacs-lisp
("mono" :font "TeX Gyre Cursor"
        :props "Color=blue,Scale=MatchLowercase")
#+END_SRC
...
As mentioned earlier, it is always advisable to use (Unicode) fonts
that cover all scripts in your document to simplify font
configuration.  If you are using =lualatex= as your LaTeX compiler,
you can also declare fallback fonts for specific scripts that are not
included in the fonts used for your document.

If the specified in the document class, or the fonts you specify don't
support a specific script, the LaTeX compiler will fail and Emacs will
warn you[fn::The warning may be overwritten by other messages in the
mini-buffer]. To help you fix the font setup, Org will include a list
of /problematic/ fonts with the Emacs scripts they don't support in
the *~*Warnings*~* buffer. For example:
...
In the example above, the warning tells you that the document class
you are using, like most LaTeX document classes, uses the Latin Modern
font family, abbreviated as ~lm...~.  These fonts are installed with
your LaTeX distribution. Most of the times, replacing these fonts by
system fonts supporting the missing scripts will be enough. Examples
for this are the Greek and Cyrillic scripts, which are supported in
font families like
[[https://www.gust.org.pl/projects/e-foundry/tex-gyre][the ~TeX Gyre~
fonts]].

However, there are scripts (for example, the ~emoji~ script in the
example above), where you need to declare the system font(s) that
provide them. These fonts, known as fallback fonts, are declared with
the ~:fallback~ property for the specific font family that needs their
support.

The ~:fallback~ property maps Emacs script names to their fallback
system font and optional extra configuration.

In the example above, you need to set the default font to ~Latin
Modern Roman~ and add a fallback font for the ~emoji~ script. In order
to allow fallback fonts, you need a substitute font for the family
that needs them.

Turning back to the example above, a possible configuration to
add the ~emoji~ script to your default font would be:
...
When Emacs detects characters belonging to the ~emoji~ character set
in the document, it will include the following fallback definition for
the ~main~ font[fn:55]:
...
(note that the ~emoji~ script is not mentioned) and, then, generate
the following font specification for =main=.

#+BEGIN_SRC latex
\setmainfont{Latin Modern Roman}[RawFeature={fallback=fallback_main}]
#+END_SRC

This will enable you to include emojis in parts of the document that
are typeset in Sans-Serif. If you have emojis in passages typeset in
Serif, you will need to add the fallback to ~main~.

The format for the ~:fallback~ specification is:
1. ~("<script>" :font "<font name>")~ to activate font when a specific
   Emacs script is detected in the buffer, or
2. ~("<script>" :font "<font name>:<extra options>")~ to activate a font
   with a set of options when a specific Emacs script is detected.

You may use any font family defined in ~org-latex-fontspec-config~.
When you specify ~:props~ for a font family, the fallback
definition is appended as another feature to the font definition.

Make sure to the fallback fonts for the LaTeX font families that need
them. You can approack ~org-latex-fontspec-config~ in a grow-as-you-go
fashion, i.e. you can use your definitions, even in the case where no
...
#+vindex: org-latex-fontspec-default-features
Additionally, you can define common features (cf. ~:props~ above) for
all fonts using the ~org-latex-fontspec-default-features~
variable. Its default value is ~nil~ meaning that no defaults are
defined. If you need default common features for all font families,
you can define them in a string in this variable. For example:

#+BEGIN_SRC elisp
(setq org-latex-fontspec-default-features "Numbers=OldStyle")
#+END_SRC

#+texinfo: @noindent
(or its equivalent as a file or directory local variable) will generate
the following line in the LaTeX preamble:

#+BEGIN_SRC latex
\defaultfontfeatures{Numbers=OldStyle}
#+END_SRC
If you need more features, add them to this string creating a
comma-separated list. (See
[[https://latex3.github.io/fontspec/user.html][the summary of
~fontspec~ commands]] in the ~babel~ guide.)

Font configuration is a "grow as you go" process. You can keep the
fallback definitions in your configuration, even if the document you
work on does not use a specific script. For example, you will normally
not use emojis in a document, but might include them in a beamer
presentation. If you use the same system fonts in both, you can use a
common font configuration for Org.
...
The appearance of a document is controlled by the fonts declared in
the *document class*. As stated above, you can provide your own fonts
and modify the appearance of the final output. If you encounter a
situation where you need to add support for a script type not
supported by the default fonts but need to keep them overall, you will
need to find the long font name for your
~org-latex-fontspec-config~. If your document class uses the =lm*=
fonts, a good translation is:
...
**** Configuration of the ~polyglossia~ package
:PROPERTIES:
:custom_id: lualatex-polyglossia
:description: Using org-latex-polyglossia-font-config
:END:
#+cindex: polyglossia font configuration
#+vindex: org-latex-polyglossia-font-config

If you use the ~polyglossia~ package in multilingual documents, it
will load the ~fontspec~ package for you. Any general settings that
depend on the variable ~org-latex-fontspec-config~ described
[[#lualatex-fontspec][above]] still apply. In addition, settings
specific to the ~polyglossia~ package are controlled by
~org-latex-polyglossia-font-config~. Its default value is ~nil~,
meaning that no extra ~polyglossia~-related font configurations are
generated.
...
definition is a property list (~plist~). The following is show an example
of the definition of the font configuration to use ~polyglossia~ in a
document with English and Hebrew text placed in a ~.dir-locals.el~ file:

#+BEGIN_SRC emacs-lisp
((org-mode
  . ((org-latex-polyglossia-font-config
      . (("en" :variant "main" :font "FreeSerif")
         ("en" :variant "sans" :font "FreeSans")
         ("en" :variant "mono" :font "FreeMono")
         ("he" :variant "main" :font "Hadasim CLM"
          :props "Script=Hebrew")
         ("he" :variant "mono" :font "Miriam Mono CLM"
          :props "Script=Hebrew")
         ("he" :variant "sans" :font "Miriam CLM"
          :props "Script=Hebrew"))))))

#+END_SRC

As you see, ~polyglossia~ controls fonts on a per-language, per-script
basis. For example, the document will use the ~Miriam Mono CLM~
script for Hebrew script in monospaced passages (~:variant "mono"~).
You will see the following in the exported LaTeX:

#+BEGIN_SRC LaTeX
  \newfontfamily{\hebrewfonttt}[Script=Hebrew]{Miriam Mono CLM}
#+END_SRC

Each entry in the property list associates a language(for example,
~"he"~ for Hebrew, ~"en"~ for English, etc.[fn:56]) to a list
of properties.
...
3. ~:props~ : a string for extra properties[fn::You can use
   ~:features~ as an alias for ~:props~.] as defined in the
   [[https://texdoc.org/serve/polyglossia.pdf/0][Polyglossia manual]].
   Square brackets are not needed, because they are added by Org.
4. ~:script~ : a string the family name, when the ~:polyglossia~
   property in ~org-latex-languages-alist~ is not appropriate (for
   exceptional cases only, cf. Note below).

When using ~polyglossia~, you can also configure the fonts used for
the document's main language with the configuration for
~fontspec~.  For example, the following settings in you
...
#+texinfo: @noindent
will set ~CMU Serif~ and ~CMU Sans Serif~ as the fonts for text and
~DejaVu Sans Mono~ for listings in all documents where you use
~fontspec~.  Additionally, if you use ~polyglossia~ and have Greek
text, you will typeset it with the ~Noto Serif~ font.

The LaTeX exporter will generate a ~\newfontfamily~ command
(cf. Chapter 4 of the
[[https://texdoc.org/serve/polyglossia/0][polyglossia manual]].)
using the
~:polyglossia~ property defined in ~org-latex-languages-alist~ for the
font family.
For example, Spanish uses the font family name
~"spanish"~, derived from the entry for ~"es"~.

*Note:* The =:polyglossia= property in ~org-latex-languages-alist~ is
used to define the font family name for the ~\newfontfamily~
definitions.  In /exceptional cases/, this property might not provide
the name expected by ~polyglossia~.  In this case, and _only_ in this
case, you might need to provide the family name you need using the
~:script~ property in ~org-latex-polyglossia-font-config~. For
example,

#+BEGIN_SRC emacs-lisp
(setq org-latex-polyglossia-font-config
...
#+texinfo: @noindent
will allow you to typeset Hindi and Sanskrit as shown in
[[https://www.overleaf.com/latex/examples/how-to-write-multilingual-text-with-different-scripts-in-latex/wfdxqhcyyjxz][this
example in overleaf.com]].  Note that
[[https://rishi.app/blog/writing-hindi-using-devanagari-package-in-overleaf/][you
don't /always/ need to provide the =:script= property]].

**** Configuration of the ~babel~ package
:PROPERTIES:
:custom_id: babel-fontconfig
:description: Using org-latex-babel-font-config
:END:
#+cindex: babel font configuration
#+vindex: org-latex-babel-font-config
#+vindex: org-latex-babel-provides-alist

Org mode will control the fonts used for multilingual documents when
using the ~babel~ package with the ~org-latex-babel-font-config~
variable.  By default, this variable is ~nil~, meaning that no extra
font configuration will be generated.

To generate a babel font configuration, this variable needs to hold a
property list array mapping Org language names to the fonts used
when exporting to babel.  Each entry maps a language name to a list of
properties.  Use ~nil~ as the language name for the fonts
used by the default language.

The properties are defined as follows:
1. ~:variant~ (mandatory) is one of the script codes. As in
   [[#lualatex-polyglossia]], they can be[fn::~babel~ uses two-letter
   codes and Org will translate four-letter codes to these two-letter
   codes as well.]:
   - ~"main"~ or ~"rm"~ for the roman (serif) font (this is normally the default
     font for text passages).
   - ~"sans"~ or ~"sf"~ for the sans serif font, and
   - ~"mono"~ or ~"tt"~ for the teletype (monospaced) font.
2. ~:font~ (mandatory) is the system font name
3. ~:props~ (optional) is a string with the options[fn::You can use
   ~:features~ as an alias to ~:props~.] to apply to this font.

For example, for a document with text in German and Greek, where the
main language is German, this variable could be initialized as
follows[fn::Using the two-letter font family codes you will find in
most examples in the Internet.]:

#+BEGIN_SRC elisp
(setq org-latex-babel-font-config
      '(("el-polyton" :variant "rm" :font "Noto Serif")
        (nil :variant "rm" :font "CMU Serif")
        (nil :variant "tt" :font "DejaVu Sans Mono"
             :props "Scale=Matchlocase")))
#+END_SRC

This setting will produce the following code in the LaTeX export
header:

#+BEGIN_SRC latex
\babelfont{rm}{CMU Serif}
\babelfont{tt}{DejaVu Sans Mono}[Scale=MatchLowercase]
\babelfont[polytonicgreek]{rm}{Noto Serif}
#+END_SRC

#+texinfo: @noindent
which will set text in Serif (~rm~) using the ~CMU Serif~ font unless
it is in polytonic Greek, where the ~Noto Serif~ font will be
used. The text set in monospaced font (~tt~) will use ~DejaVu Sans
Mono~ regardless of the language. This text will be scaled to match
the default font size and produce a better-looking output.

*Note* that the definitions for the default language (~nil~) will
apply to any document regardless of the default language it declares
to use. If you need to change the font when a specific language is
used as a secondary language, you will need to add the corresponding
entries for it.

Additionally, you can add extra properties to any language except the
default language in your document with the
~org-latex-babel-provides-alist~ variable, which maps languages to a
~:provide~ property. This property is a string. It is always
~"import,main"~ for the main language and defaults for other languages
to ~"import"~ if not present in the configuration; it will generate a
line

#+begin_example
\babelprovide[provide]{lang}
#+end_example
#+texinfo: @noindent
per language listed in ~#+LANGUAGE~. For example, if you need to
detect the language a passage is written by the script, you can use
the babel option ~import,onchar=ids fonts~.

The following settings:

#+BEGIN_SRC emacs-lisp
(setq org-latex-babel-provides-alist
      '(("el" :provide "import,onchar=ids fonts")
        ("pt" :provide "import,onchar=ids fonts")))
#+END_SRC
#+texinfo: @noindent
applied to an Org file with the following header:
...
#+END_SRC
#+texinfo: @noindent
will generate the following LaTeX preamble code:
...
#+END_SRC
#+texinfo: @noindent
assuming you have configured ~FreeSerif~ as the main font and want to
use ~DejaVu Sans~ for Portuguese with ~org-latex-babel-font-config~.

*Note* that the property for the main language (Greek in the example),
is /always/ ~import,main~.

**** Configuration of the ~xeCJK~ or ~luatexja~ packages for CJK languages
:PROPERTIES:
:CUSTOM_ID: babel-xelatex-cjk
:DESCRIPTION: Localisation and font management with xeCJK
:END:

When composing Chinese, Japanese or Korean (CJK) documents, fonts are
configured using the ~xeCJK~ package or ~luatexja~ package, depending
on the LaTeX engine you configured, while localization is provided by
~babel~.  *Note* that this solution will /only/ work with ~xelatex~
and ~lualatex~.

Setting ~org-latex-fontspec-config~ to ~nil~ will produce a working
document with the default fonts provided by the ~xeCJK~ or ~luatexja~
package.

The following document header
#+BEGIN_SRC org
,#+LANGUAGE: zh
...
#+texinfo: @noindent
will allow you to typeset a Chinese document.

Replacing ~#+LATEX_COMPILER~ option with ~lualatex~ will allow you to
typeset a Chinese document using ~luatexja~ and the default =Fandol=
fonts.

Similarly, you can typeset Japanese documents, just by replacing the
~#+LANGUAGE:~ line by
...
#+texinfo: @noindent
This will allow you to typeset a Japanese document, using the
=HaranoAjiMincho= and =HaranoAjiGothic= fonts.

The appearance of the PDF output can be further customised if needed.
The ~\setCKJmainfont{}~, ~\setCJKsansfont{}~ and ~\setCJKmonofont{}~
commands are provided as a unified interface for ~xeCJK~ and
~luatexja~ to select specific fonts.  Use ~org-latex-fontspec-config~
to define the ~CJKmain~, ~CJKsans~ and ~CJKmono~ fonts supporting the
Chinese, Japanese or Korean scripts to match your needs.  For example,
the following font configuration will produce a Chinese document
typeset with the =Noto= font family:
...
Changing the ~Noto * SC~ fonts with fonts from the ~Noto * JP~ family will
produce a Japanese document typeset with the =Noto= font family:

#+BEGIN_SRC emacs-lisp
((org-mode .
  ((org-latex-fontspec-config .
    (("CJKmain" :font "Noto Serif CJK JP")
     ("CJKsans" :font "Noto Sans CJK JP")
     ("CJKmono" :font "Noto Sans Mono CJK JP"))))))
#+END_SRC

Note that when ~lualatex~ and thus ~luatexja~ is used, setting
~CJKmono~ has no effect, while ~CJKsans~ will be used for verbatim
environment.
...
By calling a named code block[fn:57] from an Org mode buffer or
...
- =value= ::
  ...
  Default for most Babel libraries[fn:57].  Functional mode.  Org
...
Emacs[fn:60].  With Org's support, you have to indent all lines to
...
a server[fn:61] to interact with Emacs.  Pass its location through
...
Orgtbl mode inserts a radio table skeleton[fn:63] with the command
...
[fn:49] Note that ~beamer~ is yet another LaTeX class. Org provides a
specific Beamer exporter to map the Org document structure
correctly. All considerations for language and font management in
LaTeX in this document apply to the ~beamer~ exporter.

[fn:50] For example, in a document is that is intended for an American
English context where you don't need localisation, you will need to
configure the fonts you use in the PDF export, if you use emojis in
your document or intersperse words or short expressions in another
language that uses non Latin characters (e.g. Greek, Russian, etc.)

[fn:51] Org will translate the language codes to the language
designator used by ~babel~ or ~polyglossia~ automatically using
~org-latex-language-alist~. This variable maps Org language code to
different parameters used by both packages.

[fn:52] The language name is normally spelt out in lowercase. A
notable exception is Arabic in the =polyglossia= package, which needs
to be capitalised (i.e. =Arabic=).

[fn:53] If you use ~lualatex~ to compile ~beamer~ presentations, you
may see that characters used for bullets may not appear in your PDF
output. This can be remedied by using ~fontspec~ and specifying a
=math= font in ~org-latex-fontspec-config~.

[fn:54] Consult [[info:emacs#Directory Variables][Directory Variables]]
to learn to define and use directory local variables.

[fn:55] Any font in ~org-latex-fontspec-config~ which includes a
fallback for the =emoji= character set will produce equivalent code
inside the =\directlua{}= code block using the
[[https://github.com/latex3/luaotfload][luaotfload]] module.

[fn:56] The language name can be the language as written out in the
font declaration or the language identifier used in
~org-latex-languages-alist~.

[fn:57] Actually, the constructs =call_<name>()= and =src_<lang>{}=
are not evaluated when they appear in a keyword (see [[*Summary of
In-Buffer Settings]]).

[fn:58] In the following example, attempting to evaluate
...
[fn:59] Org Indent mode also sets ~wrap-prefix~ correctly for
...
[fn:60] This works, but requires extra effort.  Org Indent mode is
more convenient for most applications.

[fn:61] For a server to host files, consider using a WebDAV server,
such as [[https://nextcloud.com][Nextcloud]].  Additional help is at this [[https://orgmode.org/worg/org-faq.html#mobileorg_webdav][FAQ entry]].

[fn:62] While creating the agendas, Org mode forces =ID= properties
...
[fn:63] By default this works only for LaTeX, HTML, and Texinfo.
Configure the variable ~orgtbl-radio-table-templates~ to install
templates for other modes.

[fn:64] If the =TBLFM= keyword contains an odd number of dollar
